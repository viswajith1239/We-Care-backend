...
class AuthRepository extends baseRepository_1.default {
    constructor() {
        ...
        this._appoinmetModel = appoinmentModel_1.default;
        ...
        this._reviewModel = reviewModel_1.default;
    ...
    async existUser(email, phone) {
        try {
            let existEmail = true;
            let existPhone = true;
            const emailExist = await userModel_1.default.findOne({ email: email });
            if (!emailExist) {
                existEmail = false;
            }
            const phoneExist = await userModel_1.default.findOne({ phone: phone });
            if (!phoneExist) {
                existPhone = false;
            }
    ...
    async findUserEmail(email) {
        try {
            const hi = await this._userModel.findOne({ email }).lean();
            return hi;
        }
        catch (error) {
    ...
    async saveResetPassword(email, hashedPassword) {
        try {
            ...
            const result = await this._userModel.updateOne({ email }, { $set: { password: hashedPassword } });
    ...
    async fetchAllAppoinmentschedules() {
        try {
    ...
    async findSessionDetails(appoinmentid) {
        const response = await this._appoinmetModel.findById(appoinmentid).populate("specializationId");
        return response;
    }
    async findExistingBooking(bookingDetails) {
        try {
            ...
            if (existingBooking) {
                if (existingBooking.paymentStatus === "Cancelled") {
                    await this._bookingModel.updateOne({ _id: existingBooking._id }, { $set: { paymentStatus: "Confirmed", } });
            ...
            await this._appoinmetModel.findByIdAndUpdate(bookingDetails.appoinmentId, { isBooked: true }, { new: true });
            return null;
        }
        catch (error) {
    ...
    async createNotification(bookingDetails) {
        try {
            ...
            const doctorNotificationContent = {
                ...
                bookingId: new mongoose_1.default.Types.ObjectId(bookingDetails.appoinmentId),
                read: false,
                createdAt: new Date(),
            };
            ...
            if (existingDoctorNotification) {
                existingDoctorNotification.notifications.push(doctorNotificationContent);
                await existingDoctorNotification.save();
            }
            else {
                const newDoctorNotification = {
                    receiverId: bookingDetails.doctorId,
            ...
            if (existingUserNotification) {
                ...
                const userId = typeof bookingDetails.userId === 'object' &&
    ...
    async editUserData(userId, userData) {
        try {
            ...
            const response = await this._userModel.findByIdAndUpdate(userId, { $set: { ...updateData } }, { new: true, runValidators: true }).select('-password').lean().exec();
            return response;
        }
        catch (error) {
    ...
    async fetchNotifications(userId) {
        try {
            ...
            if (notificationsDoc?.notifications?.length) {
                notificationsDoc.notifications.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            }
    ...
    async fetchBookings(user_id, page = 1, limit = 5, search = '') {
        try {
    ...
    async addToUserWallet(userId, amount, bookingId) {
        try {
    ...
    async fetchWalletData(user_id, page = 1, limit = 5) {
        try {
    ...
    async getPrescriptionsByUser(user_id, page = 1, limit = 5, search = '') {
        try {
            const skip = (page - 1) * limit;
    ...
    async findBookings(user_id, doctor_id) {
        try {
    ...
    async createReview(reviewComment, selectedRating, userId, doctorId) {
        try {
            const data = {
    ...
    async editReview(reviewComment, selectedRating, userReviewId) {
        try {
            const review = await this._reviewModel.findByIdAndUpdate(userReviewId, { comment: reviewComment, rating: selectedRating }, { new: true });
        }
        catch (error) {
            console.error("Error creating review:", error);
            throw new Error("Failed to create review");
    ...
    async getAvgReviewsRating(doctor_id) {
        try {
            const avgRatingAndReviews = await this._reviewModel.aggregate([
                {
                    $match: { doctorId: new mongoose_1.default.Types.ObjectId(doctor_id) },
                },
                {
                    $group: {
                        _id: null,
