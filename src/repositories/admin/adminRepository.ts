...
import { IDoctor, IUser } from "../../interface/common";
...
import { DoctorResponseDTO, PaginatedDoctorResponse } from "../../dtos/doctor.dto";
...
class AdminRepository extends BaseRepository<any> implements IAdminRepository {
  ...
  async fetchAllUsers(page: number = 1, limit: number = 5,search: string = ''): Promise<PaginatedUserResponse | undefined> {
    try {
    ...
    if (search && search.trim()) {
      ...
      const users = await this._userModel.find(query).lean<UserResponseDTO[]>().skip(skip)
        .limit(limit);
  ...
  async fetchAllDoctors(page: number = 1, limit: number = 5, search: string = ''): Promise<PaginatedDoctorResponse | undefined> {
  try {
    const skip = (page - 1) * limit;
  ...
  async blockUnblockDoctor(doctor_id: string, doctorState: boolean): Promise<IDoctor> {
    const updatedDoctor = await this._doctorModel.findByIdAndUpdate(
      doctor_id,
      { isBlocked: doctorState },
      { new: true }
    ).lean<IDoctor>();
